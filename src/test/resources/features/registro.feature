Feature: Vinculacion del usuario y apertura de cuenta para gestionar sus finanzas personales
  ...
  Scenario Outline: Usuario se registra y habilita una cuenta bancaria para comenzar a administrar su dinero
    ...
    When proporciona su numero de identificacion "<numero_identificacion>", su nombre completo "<nombre_completo>" y su correo electronico "<correo_electronico>" para completar el registro
    And solicita la apertura de una cuenta con el numero "<numero_cuenta>", el saldo inicial "<saldo_inicial>" y el tipo de cuenta "<tipo_cuenta>"
